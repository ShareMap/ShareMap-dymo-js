var sharemapjs = (function(sharemapjs){
...
Polygon.prototype.collides = function(polygon) {
    var r1 = polygon.generator;
    var r2 = this.generator;

    var xInt = true;
    if (
            ((r1.x1 > r2.x2) && (r1.x2 > r2.x2))
            || ((r1.x2 < r2.x1) && (r1.x2 < r2.x1))
            )
    {
        xInt = false;
    }
    ...
    if (
            ((r1.y1 > r2.y2) && (r1.y2 > r2.y2))
            || ((r1.y2 < r2.y1) && (r1.y2 < r2.y1))
            )
    {
        yInt = false;
    }
...
Polygon.prototype.collides2 = function(polygon) {
...
Polygon.prototype.draw = function(ctx, color, opacity) {
...
Geometry.prototype.setTextLabel = function(text, x, y) {
    this.textLabel = {
        text: text,
        x: x,
        y: y
...
var LabelPoint = function(name, fontfile, fontsize, location, position, radius, properties, rank, preferred, extras) {
    this.init(name, fontfile, fontsize, location, position, radius, properties, rank, preferred, extras);
...
LabelPoint.prototype.init = function(name, fontfile, fontsize, location, position, radius, properties, rank, preferred, extras) {
    var self = this;
    if (!rank)
        rank = 1;
    if (!preferred)
    ...
    if (full_extras) {
        ...
        self.populate_placements(preferred)
...
LabelPoint.prototype.populate_shapes = function() {
    metrics = {};
    metrics['New York'] = {w: 56.000000, h: 10.000000, b: 2.000000};
    metrics['Philadelphia'] = {w: 84.000000, h: 13.000000, b: 2.000000};
    var self = this;
    // Set values for self.label_shapes,footprint_shape, and others.
    //
    //point = geometry.LabelPoint.self.position.x, self.position.y)

    //point_buffered = point.buffer(self.radius + self.buffer, 3)
    //self.point_shape = point.buffer(self.radius, 3)
    ...
    for (var pKey in LabelPoint.placements) {
        ...
        var placement = pKey;
        var label_shape = this.label_bounds(x, y, w, h, 0, placement);
        label_shape.name = "label_shape " + self.name;
        var label_shape_buffered = label_shape.buffer(self.buffer, 2)
        var mask_shape = label_shape;//label_shape_buffered.union(point_buffered);
...
LabelPoint.prototype.populate_placements = function(preferred) {
    var self = this;
...
LabelPoint.prototype.overlaps = function(other, reflexive) {
    ...
    var self = this;
    overlaps = self.mask_shape.intersects(other.label())
...
/*
 LabelPoint.prototype.label_bounds = function(orgX, orgY, width, height, radius, placement) {
 var x = orgX;
 var y = orgY;
 ...
 res.setColor("red");
 res.setTextLabel(this.name, x1, y1);
 return res;
...
sharemapjs.Places = Places;var Annealer = function(energyFunc, moveFunc) {
    this.init(energyFunc, moveFunc);
};

Annealer.prototype.init = function(energyFunc, moveFunc)
{
    this.fakeData = (sharemapjs.fakeData === true);
    var self = this;
...
sharemapjs.Annealer = Annealer;var Labeler = function() {
}
